...
class PGAgent(nn.Module):
    ...
    def _estimate_advantage(
        ...
        if self.normalize_advantages:
            advantages = (advantages - np.mean(advantages, axis=1)) / (np.std(advantages, axis=1) + 1e-8)
    ...
    def _discounted_return(self, rewards: Sequence[float]) -> Sequence[float]:
        ...
        sum_rewards = np.sum(rewards * discounts)
        return np.repeat(sum_rewards, len(rewards))
    ...
    def _discounted_reward_to_go(self, rewards: Sequence[float]) -> Sequence[float]:
        ...
        for i in range(len(rewards)):
            discounts = np.array([self.gamma ** j for j in range(i, len(rewards))])
            sum_rewards = np.sum(rewards[i:] * discounts)
            q_value.append(sum_rewards)
        return np.array(q_value)
